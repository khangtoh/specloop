...
- _2026-09-13_ — 0.5.0 release attempted and blocked by npm two-factor authentication (`E403` on publish). Nothing was tagged or pushed and the registry still serves 0.4.0; the 0.5.0 bump in `package.json` is committed. Resume: `npm publish --otp=<code>` from the repo root, then `git tag v0.5.0 && git push --follow-tags origin main`. Do not re-run `bun run release minor`, which would bump to 0.6.0.
